    ...
    <section className="container hero-grid" style={{
      ...
      <div style={{ textAlign: 'left' }}>
        ...
        <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', marginBottom: '2.5rem', lineHeight: 1.7 }}>
          Potenciamos federaciones con una solución digital unificada: controle el padrón institucional e inscripciones con <strong>SIGDEF</strong> y transmita tiempos de regatas en tiempo real con <strong>SportTrack</strong>.
        </p>
        <div className="hero-btn-group" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <a href="#servicios" className="btn-primary" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
            Ver Planes <ArrowRight size={16} />
          </a>
          <a href="#sistemas" className="btn-secondary" style={{ textDecoration: 'none' }}>Nuestros Sistemas</a>
        </div>
      </div>

      <div className="glass-card" style={{ padding: '1rem', overflow: 'hidden' }}>
        <img src={dashboardImg} alt="Dashboard SIGDEF y SportTrack - Panel del Ecosistema" style={{ width: '100%', borderRadius: '1rem', display: 'block' }} />
      </div>
    </section>
  );
}
